...
class FilePicker {
 public:
  ...
  // GYY
  void PrintFileAccessCount() {
    const std::string filename = "/home/gengyiyang/a.txt";  // 修改为绝对路径
    std::ofstream outfile(filename, std::ios::app);  // 追加模式打开文件

    if (outfile.is_open()) {
      for (const auto& pair : file_access_count_) {
        FdWithKeyRange* file = pair.first;
        int access_count = pair.second;
        // Print or process access count for the file
        outfile << "File: " << file->fd.GetNumber()
                << ", Access Count: " << access_count
                << ", Level: " << GetCurrentLevel() << std::endl;
      }
      outfile.close();
    } else {
 ...
 private:
  ...
  std::unordered_map<FdWithKeyRange*, int> file_access_count_;
  //=====================================================================================
