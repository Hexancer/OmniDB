...
class FollySkipListTest : public ::testing::Test {
 protected:
  void SetUp() override { skiplist = new FollySkipList(32, &cmp, 1ULL << 20); }
 ...
 protected:
  ...
  void CheckAllNodes(std::vector<std::string> expected) {
    ...
    // remove sentinels
    auto it = std::remove(expected.begin(), expected.end(), SENTINEL_STR);
...
TEST_F(FollySkipListTest, Basic4) {
  InsertRange(10, 2);
  InsertRange(15, 2);
  CheckAllNodes({"10", "11", SENTINEL_STR, "15", "16", SENTINEL_STR});
}
...
TEST_F(FollySkipListTest, BasicNext1) {
  InsertRange(10, 2);
  InsertRange(15, 2);

  auto it = skiplist->NewIterator();
  it->Seek("0");
...
TEST_F(FollySkipListTest, BasicNext2) {
  InsertRange(10, 2);
  InsertRange(15, 2);

  skiplist->Insert("12", "12");
  skiplist->Append("13", "13");
  skiplist->Append("14", "14");
  skiplist->Append("15", "15");
...
TEST_F(FollySkipListTest, BasicOverlap1) {
  ...
  CheckAllNodes({"10", "11", SENTINEL_STR, "15", "16", SENTINEL_STR});
  AppendRange(12, 4);
  CheckAllNodes({"10", "11", "12", "13", "14", "15", "16", SENTINEL_STR});
}
...
TEST_F(FollySkipListTest, BasicOverlap2) {
  ...
  CheckAllNodes({"10", SENTINEL_STR});
  InsertRange(11, 1);
  CheckAllNodes({"10", SENTINEL_STR, "11", SENTINEL_STR});
  AppendRange(101, 2);
  CheckAllNodes({"10", "101", "102", SENTINEL_STR, "11", SENTINEL_STR});
  InsertRange(130, 1);
  CheckAllNodes({"10", "101", "102", SENTINEL_STR, "11", SENTINEL_STR, "130",
                 SENTINEL_STR});
}
