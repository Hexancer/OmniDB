...
#ifndef ROCKSDB_EXP_SMO_H
#define ROCKSDB_EXP_SMO_H
...
class es_vec {
 ...
 public:
  ...
  /** Operator [] */
  T& operator[](const size_t _i) { return esv_data[_i]; }

  const T& operator[](const size_t _i) const { return esv_data[_i]; }
  ...
  /** Operator + */
  friend es_vec<T, D> operator+(const es_vec<T, D>& _vec1,
                                const es_vec<T, D>& _vec2) {
  ...
  /** Operator == */
  friend  bool operator==(const es_vec<T, D>& _vec1,
                               const es_vec<T, D>& _vec2) {
    for (size_t i = 0; i < D; ++i) {
...
class single_exponential_smoothing {
 public:
  ...
  single_exponential_smoothing()
      : ses_smoothing_const(0.5), ses_counter(0), ses_vacillation_tolerance(0) {
 ...
 public:
  ...
  const es_vec<T, D> push_to_pop(const es_vec<T, D> _curr_raw_ob) {
    if (0 < ses_counter) {
      ...
      ses_curr_smoothed_ob = ses_smoothing_const * _curr_raw_ob +
                             (1 - ses_smoothing_const) * ses_curr_smoothed_ob;

      if ((ses_curr_smoothed_ob - temp_last_smoothed_ob).norm() <
          ses_vacillation_tolerance) {
  ...
  /** Get the current counter */
   size_t counter() const { return ses_counter; }
 ...
 public:
  ...
  const es_vec<T, D> push_to_pop(const es_vec<T, D> _curr_raw_ob) {
    if (0 == des_counter) {
      ...
      des_curr_smoothed_ob =
          des_1st_smoothing_const * _curr_raw_ob +
          (1 - des_1st_smoothing_const) *
              (des_curr_smoothed_ob + des_curr_correction_ob);

      if ((des_curr_smoothed_ob - des_last_smoothed_ob).norm() <
          des_vacillation_tolerance) {
        des_curr_smoothed_ob = des_last_smoothed_ob;
      }
    } else {
      des_curr_correction_ob =
          des_2nd_smoothing_const *
              (des_curr_smoothed_ob - des_last_smoothed_ob) +
  ...
  /** Get the current counter */
   size_t counter() const { return des_counter; }
