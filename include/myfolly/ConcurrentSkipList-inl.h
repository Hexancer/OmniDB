...
template <typename T>
// ESElmeent
class SkipListNode: public rocksdb::LRUElement<SkipListNode<T>> {
  enum : uint16_t {
    IS_HEAD_NODE = 1,
 ...
 public:
  ...
  SkipListNode* copyHead(SkipListNode* node) {
    ...
    auto succ0 = node->skip(0);
    setSkip(0, succ0);
    succ0->setBack(this);
    for (uint8_t i = 1; i < node->height_; ++i) {
      setSkip(i, node->skip(i));
    }
    return this;
  }

  inline SkipListNode* back() const {
  ...
  void setBack(SkipListNode* back) {
