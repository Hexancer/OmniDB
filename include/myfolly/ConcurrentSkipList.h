...
#include <myfolly/ConcurrentSkipList-inl.h>
...
class ConcurrentSkipList {
 ...
 public:
  ...
  explicit ConcurrentSkipList(const T& data, int height)
      : recycler_(),
        head_(NodeType::create(recycler_.alloc(), height, data, true)) {}
  ...
  static Accessor create(T&& data, int height = 1) {
    return Accessor(createInstance(height, data));
  }
  ...
  static std::shared_ptr<SkipListType> createInstance(const T& data,
                                                      int height = 1) {
    return std::make_shared<ConcurrentSkipList>(data, height);
  }

  NodeType* createNode(const T& data, int height) {
    return NodeType::create(recycler_.alloc(), height, data);
  }
 ...
 public:
  ...
  template <typename U>
  std::pair<NodeType*, size_t> addOrGetData(U&& data, bool doAppend) {
    ...
    while (true) {
      ...
      auto pred0 = preds[0];
      auto succ0 = succs[0];
      newNode->setSkip(0, succ0);
      newNode->setBack(pred0);
      succ0->setBack(newNode);
      pred0->setSkip(0, newNode);
      for (int k = 1; k < nodeHeight; ++k) {
        newNode->setSkip(k, succs[k]);
        preds[k]->setSkip(k, newNode);
      }

      if (doAppend) {
        pred0->data().sentinel_ = false;
      }
  ...
  bool remove(const value_type& data) {
    ...
    while (true) {
      ...
      for (int k = nodeHeight - 1; k >= 1; --k) {
        preds[k]->setSkip(k, nodeToDelete->skip(k));
      }
      auto succ0 = nodeToDelete->skip(0);
      preds[0]->setSkip(0, succ0);
      succ0->setBack(preds[0]);
  ...
  NodeType* seek(const value_type& data) const {
    NodeType* pred = head_.load(std::memory_order_acquire);
    int ht = pred->height();
    NodeType* node = nullptr;
...
class ConcurrentSkipList<T, Comp, NodeAlloc, MAX_HEIGHT>::Accessor {
 ...
 public:
  ...
  std::pair<iterator, bool> insert(U&& data) {
    auto ret = sl_->addOrGetData(std::forward<U>(data), false);
  ...
  std::pair<key_type*, bool> addOrGetData(const key_type& data) {
    auto ret = sl_->addOrGetData(data, false);
  ...
  bool add(const key_type& data) { return sl_->addOrGetData(data, false).second; }
